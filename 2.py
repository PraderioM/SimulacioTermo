...
dv = 10.
for v in arange(-3000.,3001.+dv,dv): # theoretical prediction
    theory.plot(pos=(v,
        deltav*Natoms*sqrt(Matom/(2.*pi*k*T))
                     *exp((-0.5*Matom*v**2)/(k*T))))

observation = ghistogram(bins=arange(-3000.,3000.,deltav),
...
while True:
    ...
    pxlist = [v[0]/Matom for v in p]
    px = array(pxlist)
    observation.plot(data=px)
