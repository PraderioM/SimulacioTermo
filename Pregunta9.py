...
rm=2**(1./6)
u=lambda r: 4*((1/r)**12-(1/r)**6)
e=abs(u(rm))
B2T=lambda T: 2*pi*integrate.quad(lambda r: (1-exp(-u(r)/T))*r**2, 0, inf)[0]
aT=lambda T: 2*pi*integrate.quad(lambda r: (1-exp(-(u(r)+e)/T))*r**2, 0, rm)[0]
bT=lambda T: 2*pi*integrate.quad(lambda r: (1-(1-(u(r)+e)/T)*exp(-(u(r)+e)/T))*r**2, 0, rm)[0]

f=lambda d: d*kT+B2*kT*d**2+(a*kT*d**2)*(8*(8-b*d)/(4-b*d)**3-1)
g=lambda kT, B2, a, b: d*kT+B2*kT*d**2+(a*kT*d**2)*(8*(8-b*d)/(4-b*d)**3-1)
...
for i in range(8):
	...
	a=aT(kT)
	b=bT(kT)
	aux, =plt.plot(X, [f(d) for d in X], label=Label[i], color=Colors[i])
	curvas.append(aux)
...
for i in range(8):
	d=D[0][i]
	aux, =plt.plot(X, [g(kT, B2T(kT), aT(kT), bT(kT)) for kT in X], label=Label[i], color=Colors[i])
	curvas.append(aux)
